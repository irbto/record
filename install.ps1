...
$binaryAsset = $release.assets | Where-Object name -eq $assetName | Select-Object -First 1
...
$binaryPath = Join-Path $temporaryRoot $assetName
$checksumPath = "$binaryPath.sha256"

try {
    New-Item -ItemType Directory -Path $temporaryRoot | Out-Null
    Invoke-WebRequest -Uri $binaryAsset.browser_download_url -Headers $headers -OutFile $binaryPath
    ...
    $actual = (Get-FileHash -Algorithm SHA256 $binaryPath).Hash
    if ($actual -ine $expected) {
        throw "SHA-256 verification failed for $assetName."
    }

    New-Item -ItemType Directory -Force -Path $InstallDirectory | Out-Null
    Copy-Item -LiteralPath $binaryPath -Destination (Join-Path $InstallDirectory $assetName) -Force
    ...
    Write-Host "Run now: record" -ForegroundColor Cyan
} finally {
